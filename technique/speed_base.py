...
details_individu_df = pd.DataFrame({
    ...
    'strasbourg_piscine_quotient_familial': np.repeat(entree_unitaire.qf, entree_unitaire.quantité),
...
sample_individu_df = pd.DataFrame({
    ...
    'strasbourg_piscine_quotient_familial': np.tile(details_individu_df.strasbourg_piscine_quotient_familial, sample_count),
    'age': np.tile(details_individu_df.age, sample_count),
    'strasbourg_piscine_entree_unitaire': np.tile(details_individu_df.strasbourg_piscine_entree_unitaire, sample_count),
})
...
tf = tempfile.NamedTemporaryFile(delete = False, prefix="speed_base_", suffix=f"_{sample_count}.profile", dir=".")
